...
| `pid_target` | 40 | **40.04** | 半步补偿（详见步骤 1）；若加 `pid_target_offset` 配置项则写在那里 |
| `pid_cycle` | 60 | **25** | 纹波带 ∝ 周期；短于 ~25 s 会因「1 帧 = 1/周期 的占空比颗粒」变粗而变差 |
| `pid_kd` | 0.5 | 0.5（不动） | 安静探头下影响可忽略 |

效果（12 h 仿真：安静探头 / 含 ±0.02 慢抖与 0.05 尖峰）：显示 40.0 占比 **86.3% → 98.6%**（含抖动 94.7%），
下发次数 985/h → **412~463/h**（反而减半）。
已试过把 PWM 换成 ΔΣ 调制：只多 +0.5 pp 但下发翻倍 → 不采用。
