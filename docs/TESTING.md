...
| `pid_cycle` | 60 | **25** | 纹波带 ∝ 周期；再短会因「1 帧 = 1/周期 的占空比颗粒」变粗而变差 |
| `pid_kd` | 0.5 | **0** | 量化台阶下微分项只放大卡点噪声（含抖动时 83.1% → 79.2%） |

效果（12 h 仿真：安静探头 / 含 ±0.02 慢抖与 0.05 尖峰）：显示 40.0 占比 **6.6% → 96.5%**（含抖动 83.1%），
下发次数 985/h → **288/h**（= 硬滞环上限 2/周期）。

**开关次数 ↔ 显示占比（机械继电器选工作点用）**

硬滞环（见下）把开关次数锁到 `2/pid_cycle`，于是「稳在目标」与「少开关」成为**显式可选的权衡**（12 h、安静探头）：
...
硬规律：**纹波(℃) × 开关次数(/h) ≈ 14.8**（PWM 与「滞环开关」两种调制交叉验证）—— 想稳在 40.0 就必须多开关，省开关就得接受大波动。
设备是机械继电器时可按触点寿命预算（额定次数 ÷ 年运行小时）从表里选周期。

**硬滞环（机械继电器必需）**：本周期一旦关掉就**锁定不再开**，且关断后 ≥3 s 才允许再开。
没有它时 duty 在量化台阶边缘抖动会把继电器帧级反复吸合（实测 750~985/h，且**把周期拉长到 300 s 也压不下来**）；
加上后开关次数 = 2/周期 且可精确预测。代价是修正延后到下一周期（靠 `δ` 补偿；`δ=0` 时真值会挂低 0.1 ℃）。

已试过但**不采用**：① PWM 改 ΔΣ 调制（按欠账导通、导通段 1 帧）：+0.5 pp 但下发翻倍（988/h）；
② 「开 1s 关 9s」这类极小周期：为同一权衡的另一端（987/h、96.5%）。
