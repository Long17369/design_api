...
- 临时产物清理（可选，随时可做）：脚本每次运行会写 `tmp/*_result.json`、启动日志 `tmp/server*.log` ——
  ```bash
  rm -f tmp/*.log tmp/*.txt tmp/*.json tmp/test_*.ts   # 只删产物，保留 e2e_*.mjs / *.ts 脚本
  ```
  保留项：`e2e_*.mjs`、`e2e_blocked_bus.ts`、`check_lock_{a,b}.mjs`（重启恢复验证）、
  `verify_seeds.ts` / `print_configs.ts`（排查工具）。
- 注意：
