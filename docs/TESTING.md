...
| `tests/autoControl/autoConfig.test.ts`       | 阈值配置读取：功能开关默认值（等于加开关前的现状）、`direct` 设备值 > 默认值 > 内置默认、空串回退                                                                \|\| `tests/autoControl/dryBurn.test.ts`          \| 干烧保护：加热累计时长阈值（含 PWM 断续只算导通段）、加热速度边界与负值、缺测/关开关不判定、锁定后不重复、复位后可再判定、优先级高于温控 \|\| `tests/sensorModule/spike.test.ts`           \| 跳变阈值边界、关闭字段、缺测不误判 |
